...
std::array<uint8_t, 32> sha256(const std::vector<uint8_t> &data) {
    ...
    for (size_t chunkStart = 0; chunkStart < message.size(); chunkStart += 64) {
        uint32_t w[64];
        for (int i = 0; i < 16; ++i) {
            w[i] = (static_cast<uint32_t>(message[chunkStart + static_cast<size_t>(i) * 4]) << 24) |
                   (static_cast<uint32_t>(message[chunkStart + static_cast<size_t>(i) * 4 + 1]) << 16) |
                   (static_cast<uint32_t>(message[chunkStart + static_cast<size_t>(i) * 4 + 2]) << 8) |
                   (static_cast<uint32_t>(message[chunkStart + static_cast<size_t>(i) * 4 + 3]));
        }
        for (int i = 16; i < 64; ++i) {
